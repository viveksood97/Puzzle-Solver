...
class Tile:
    ...
    def __init__(self,testCase,visited,size):
        ...
        self.results = []
        self.goal = np.append(np.array([i for i in range(1,16)]),0)
...
def main():
    ...
    for i in tile.results:
        print(np.reshape(i,(sizeOfProblem,sizeOfProblem)),"\n       \u2193")
    print(np.reshape(tile.goal,(sizeOfProblem,sizeOfProblem)),"     Solved!!!!!!!!!!")

    print(f"\nPuzzle solved in {round(end - start, 4)} seconds with {steps} steps.\n")
    print("Note: The last step seen above is not actually counted by the algoritm as it is the goal matrix.\n")
    print(f"If we count the last step then the number of step are '{steps +  1}'\n")
